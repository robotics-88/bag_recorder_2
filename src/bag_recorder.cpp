...
BagRecorder::BagRecorder(rclcpp::executors::MultiThreadedExecutor &executor)
    : Node("bag_recorder"), executor_(executor) {
    record_service_ = this->create_service<bag_recorder_2::srv::Record>(
        "~/record", std::bind(&BagRecorder::trigger_recording, this, _1, _2));
}

void BagRecorder::trigger_recording(
    const std::shared_ptr<bag_recorder_2::srv::Record::Request> request,
...
void BagRecorder::start_recording(std::string config, std::string data_directory) {
    ...
    recorder_ = std::make_shared<rosbag2_transport::Recorder>(std::move(writer), storage_options,
                                                              record_options);
...
bool BagRecorder::load_config(std::string config_file, std::vector<std::string> &topics,
    ...
    if (loaded.find(config_file) == loaded.end()) {
        ...
        RCLCPP_WARN(this->get_logger(), "%s config loaded already, circular reference detected.",
                    config_file.c_str());
        return false;
    }
